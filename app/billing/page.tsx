...
export default function BillingPage() {
  ...
  const handleCancelSubscription = async () => {
    if (
      !confirm(
        "本当にサブスクリプションをキャンセルしてフリープランに戻りますか？"
      )
    ) {
      return;
    }
    ...
    console.log(
      "📊 Current subscription info before cancellation:",
      subscriptionInfo
    );
    ...
    try {
      ...
      if (error.message) {
        ...
        if (error.message.includes("No active subscription")) {
          errorMessage =
            "アクティブなサブスクリプションが見つかりません。既にキャンセル済みの可能性があります。";
        } else if (error.message.includes("authentication")) {
          errorMessage =
            "認証エラーが発生しました。再度ログインしてお試しください。";
        } else if (
      ...
      try {
        ...
        console.error(
          "❌ Failed to reload subscription info after error:",
          reloadError
        );
      }
    } finally {
        ...
        <div className="max-w-7xl mx-auto px-6 py-16">
          ...
          {subscriptionInfo?.trial?.isTrialing && (
            <div className="mb-8 p-6 rounded-2xl bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 max-w-2xl mx-auto">
              ...
              {subscriptionInfo.trial.trialEndDate && (
                <div className="text-sm text-blue-700">
                  <div className="flex justify-between pt-2 border-t border-blue-200">
                    <span>トライアル終了予定:</span>
                    <span className="font-semibold">
                      {subscriptionInfo.trial.trialEndDate.toLocaleDateString(
                        "ja-JP"
                      )}
                    </span>
                  </div>
          ...
          {subscriptionInfo?.grandfathered?.isGrandfathered &&
            subscriptionInfo.grandfathered.limits && (
              <div className="mb-8 p-6 rounded-2xl bg-gradient-to-r from-amber-50 to-yellow-50 border border-amber-200 max-w-2xl mx-auto">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-amber-700">
                      {subscriptionInfo.grandfathered.grandfatheredPlan ||
                        "上位プラン"}
                      の利用制限を継続してご利用いただけます
                    </p>
                  </div>
                ...
                <div className="space-y-2 text-sm text-amber-700">
                  <div className="flex justify-between">
                    <span>面接練習:</span>
                    <span className="font-semibold">
                      {subscriptionInfo.grandfathered.limits.interviews || 0}
                      回/月
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span>ES添削:</span>
                    <span className="font-semibold">
                      {subscriptionInfo.grandfathered.limits.esCorrections || 0}
                      回/月
                    </span>
                  </div>
                  {subscriptionInfo.grandfathered.expiresAt && (
                    <div className="flex justify-between pt-2 border-t border-amber-200">
                      <span>特典終了予定:</span>
                      <span className="font-semibold">
                        {subscriptionInfo.grandfathered.expiresAt.toLocaleDateString(
                          "ja-JP"
                        )}
                      </span>
                    </div>
            ...
            <div>
                    ...
                    {subscriptionInfo?.plan === "free" ? (
                      ...
                      <button
                        onClick={(e) => {
                          ...
                          e.stopPropagation();
                          console.log(
                            "🚨 DIRECT BUTTON CLICK - bypassing any form submission"
                          );
              ...
              <div className="max-w-4xl mx-auto mt-12 text-center">
                <p className="text-gray-600 text-sm">
                  プランの変更や請求サイクルについて詳しく知りたい方は、
                  <a
                    href="/contact"
                    className="text-[#163300] hover:text-[#9fe870] underline font-medium"
                  >
                    よくある質問ページ
                  </a>
                  をご確認ください。
